 ...
 *
 * Reads: data/backtest-v2/events-enriched-merged.jsonl (output of v1 enricher)
 * Writes: data/backtest-v2/events-enriched-v2-merged.jsonl
 *
...
const IN_FILE = path.join(ROOT, "data", "backtest-v2", "events-enriched-merged.jsonl");
const OUT_FILE = path.join(ROOT, "data", "backtest-v2", "events-enriched-v2-merged.jsonl");
...
function addFlowFeatures(ev) {
  ...
  // Map CFD sym to ETF for flow lookup
  const flowSym = ev.sym === 'US30' ? 'DIA' : ev.sym === 'XAUUSD' ? 'GLD' : ev.sym;
  const accAtStrike = getFlowUpToHour(flow, flowSym, ev.strike, touchHour);
  const daySym = getFlowDaySym(flow, flowSym);
